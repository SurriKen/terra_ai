...
def выполнить_команду(команда='!ls'):
    proc = subprocess.Popen(f'{команда}', shell=True, stdin=None, stdout=open(os.devnull, "wb"), stderr=STDOUT,
                            executable="/bin/bash")
    proc.wait()
    pass
...
from . import датасет, модель, сегментация, traide, ряды, генетика, повышение_размерности, обнаружение, \
    обработка_текста, сегментация_договоров, квартиры, home
...
def обучение_модели_квартиры(мод, x_train, y_train, x_test=None, y_test=None, размер_пакета=None, количество_эпох=None,
                             коэф_разделения=0.2, инструменты=None):
    модель.обучение_модели_квартиры(мод, x_train, y_train, x_test, y_test, размер_пакета, количество_эпох,
                                    коэф_разделения, инструменты)


def обучение_модели(мод, x_train, y_train, x_test=None, y_test=None, размер_пакета=None, количество_эпох=None,
                    коэф_разделения=0.2):
...
def тест_модели_торговли(нейронка, тестовая_выборка, метки_тестовой_выборки, данные, период_предсказания,
                         количество_анализируемых_дней):
    traide.model_test(
        нейронка,
